...
bool solver(vector<vector<char>> &board, vector<pair<int, int>> &empty, int idx,
  ...
  for (int i = 0; i < 9; i++) {
    int mask = (1 << i);
    if (!((rowMask[r] & mask) || (colMask[c] & mask) ||
          (boxMask[box] & mask))) {
      board[r][c] = (char)('1' + i);
      rowMask[r] |= mask;
      colMask[c] |= mask;
      ...
      if (solver(board, empty, idx + 1, rowMask, colMask, boxMask))
        return true;
