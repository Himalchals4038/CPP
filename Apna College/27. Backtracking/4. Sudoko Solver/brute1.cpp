...
bool isSafe(vector<vector<string>> &board, int row, int col, int val) {
  for (int i = 0; i < 9; i++) {
    if (board[row][i] == to_string(val))
      return false;
  }
  for (int i = 0; i < 9; i++) {
    if (board[i][col] == to_string(val))
      return false;
  }
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      if (board[i + startRow][j + startCol] == to_string(val))
        return false;
    }
  }
  return true;
...
void sudokuSolver(vector<vector<string>> &board) { solver(board, 0, 0); }
int main() {
